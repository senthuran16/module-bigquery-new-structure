...
function validateMandatoryFields(jwt:JwtPayload jwtPayload) returns boolean {
    if (jwtPayload["iss"] == "" || jwtPayload["sub"] == "" ||
        jwtPayload["exp"] == 0 || jwtPayload["aud"].toString().length() == 0) {
        return false;
    }
    return true;
}

function validateMandatoryJwtHeaderFields(jwt:JwtHeader jwtHeader) returns boolean {
    if (jwtHeader["alg"] == "") {
        return false;
    }
    return true;
...
function encodeBase64Url(byte[] valueToEncode) returns string {
    string encodedString = encoding:encodeBase64Url(valueToEncode);
    encodedString = stringutils:replaceAll(encodedString, "[+]+", "-");
    encodedString = stringutils:replaceAll(encodedString, "/", "_");
