...
function convertToProjectsList(json jsonProjectList) returns ProjectList {
    ...
    projectList.totalItems = convertToInt(jsonProjectList.totalItems.toString());
    return projectList;
}
...
function convertToInt(json jsonVal) returns int {
    ...
    if (stringVal != "") {
        var intVal = int.constructFrom(stringVal);
        if (intVal is int) {
            return intVal;
        } else {
...
function convertToFloat(json jsonVal) returns float {
    string stringVal = jsonVal.toString();
    if (stringVal != "") {
        var floatVal = float.constructFrom(stringVal);
...
function convertToBoolean(json jsonVal) returns boolean {
    string stringVal = jsonVal.toString();
    if (stringVal != "") {
        var booleanVal = boolean.constructFrom(stringVal);
        if (booleanVal is boolean) {
            return booleanVal;
        } else {
            error err = error(BIGQUERY_ERROR_CODE,
            message = "Error occurred when converting " + stringVal + " to boolean");
            panic err; 
        }
    } else {
        return false;
...
function convertToTableData(json jsonTableData) returns TableData {
    ...
    tableData.totalRows = jsonTableData.totalRows != null ? convertToInt(jsonTableData.totalRows.toString()) : 0;
...
function convertToInsertError(json[] jsonInsertErrors) returns InsertError[] {
    ...
    foreach json jsonInsertError in jsonInsertErrors {
        insertError.index = jsonInsertError.index != null ? convertToInt(jsonInsertError.index.toString()) : 0;
...
function convertToTableList(json jsonTableList) returns TableList {
    ...
    tableList.totalItems = jsonTableList.totalRows != null ? convertToInt(jsonTableList.totalRows.toString()) : 0;
...
function convertToQueryResults(json jsonQueryResults) returns QueryResults {
    ...
    queryResults.jobComplete = jsonQueryResults.jobComplete != null ?
                convertToBoolean(jsonQueryResults.jobComplete.toString()) : false;
